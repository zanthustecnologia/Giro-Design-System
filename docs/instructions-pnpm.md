  ...
  pnpm --filter @giro/tokens build
  pnpm --filter @giro/react build
  pnpm --filter @giro/utilities build

  # Forma curta (com -F)
  pnpm -F @giro/tokens build
  ```
  ...
  pnpm --filter @giro/react add react-icons

  # Adicionar como dependência de dev
  pnpm --filter @giro/tokens add -D sass

  # Adicionar em múltiplos pacotes
  pnpm --filter @giro/* add lodash
  ```
  ...
  pnpm --filter @giro/react remove package-name
  ```
  ...
  pnpm --filter @giro/react update react
  ```
  # Atualizar em um workspace específico
  ---
